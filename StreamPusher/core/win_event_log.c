...
void sp_cdecl
sp_event_log(sp_err_t err, const char *fmt, ...)
{
	u_char         *p, *last;
	long            types;
	HKEY            key;
	HANDLE          ev;
	va_list         args;
	u_char          text[SP_MAX_ERROR_STR];
	const char     *msgarg[9];
	static u_char   netmsg[] = "%SystemRoot%\\System32\\netmsg.dll";

	last = text + SP_MAX_ERROR_STR;
	p = text + GetModuleFileName(NULL, (char *)text, SP_MAX_ERROR_STR - 50);
	...
	*p++ = ':';
	...
	if (p > last - SP_LINEFEED_SIZE - 1) {
		p = last - SP_LINEFEED_SIZE - 1;
	}
